...
export class ModerationService {
  ...
  async notifyOperators(moderationId: string): Promise<void> {
    try {
      ...
      const techPhotoLink = techPhotoFullUrl ? `📸 <a href="${techPhotoFullUrl}">Tex pasport old tomoni</a>\n` : '';
      const techBackPhotoLink = techBackPhotoFullUrl ? `📸 <a href="${techBackPhotoFullUrl}">Tex pasport orqa tomoni (seriyali)</a>\n` : '';
      const carPhotoLink = carPhotoFullUrl ? `📸 <a href="${carPhotoFullUrl}">Mashina rasmi</a>\n` : '';
      const secondPhoneText = data.secondPhone ? `📞 Ikkinchi telefon: <code>${this.formatPhone(data.secondPhone)}</code>\n` : '';

      const message = `━━━━━━━━━━━━━━━━━━\n🔄 <b>YANGI AVTOMOBIL MODERATSIYASI</b>\n━━━━━━━━━━━━━━━━━━\n\n📋 <b>MA'LUMOTLAR:</b>\n━━━━━━━━━━━━━━━━━━\n🚗 Avtomobil raqami: <b>${data.plateNumber}</b>\n👤 Avtomobil egasi: <b>${data.ownerName}</b>\n📞 Asosiy telefon: <code>${this.formatPhone(data.ownerPhone)}</code>\n${secondPhoneText}━━━━━━━━━━━━━━━━━━\n📅 Sug'urta turi: <b>${insuranceTypeText}</b>\n📆 Boshlanishi: ${new Date(data.startDate).toLocaleDateString('uz-UZ')}\n📆 Tugashi: ${new Date(data.endDate).toLocaleDateString('uz-UZ')}\n━━━━━━━━━━━━━━━━━━\n\n📸 <b>RASMLAR:</b>\n${techPhotoLink}${techBackPhotoLink}${carPhotoLink}━━━━━━━━━━━━━━━━━━\n\n👤 Registrar: <b>${data.registrarName}</b>\n📅 Sana: ${new Date(data.submittedAt).toLocaleString('uz-UZ')}\n━━━━━━━━━━━━━━━━━━\n\nTasdiqlaysizmi?`;
      ...
      for (const operator of operators) {
        try {
          ...
          this.logger.error(`❌ Operator #${operator.id} ga xabar yuborishda xatolik: ${errorMessage}`);
        }
      }
      this.moderationStorage.set(moderationId, moderation);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Operatorlarga xabar yuborishda xatolik: ${errorMessage}`);
    }
  }
  ...
  async markNotified(moderationId: string, operatorId: number): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ markNotified xatosi: ${errorMessage}`);
    }
  }
  ...
  async notifyApproval(moderationId: string, car: any): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Tasdiqlash xabarini yuborishda xatolik: ${errorMessage}`);
    }
  }
  ...
  async notifyRejection(moderationId: string, reason: RejectionReason): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Rad etish xabarini yuborishda xatolik: ${errorMessage}`);
    }
  }
  ...
  async approve(id: string, operatorId: number): Promise<{ success: boolean; car?: Car; message: string }> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Moderatsiyani tasdiqlashda xatolik: ${errorMessage}`);
      return { success: false, message: '❌ Xatolik yuz berdi' };
    }
  }

  async reject(id: string, operatorId: number, reason: RejectionReason): Promise<{ success: boolean; message: string }> {
    try {
      const moderation = await this.get(id);
      if (!moderation) {
        return { success: false, message: '❌ Moderatsiya topilmadi yoki muddati o\'tgan' };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Moderatsiyani rad etishda xatolik: ${errorMessage}`);
      return { success: false, message: '❌ Xatolik yuz berdi' };
    }
  }
  ...
  private async remindOperators(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Noma\'lum xatolik';
      this.logger.error(`❌ Eslatma yuborishda xatolik: ${errorMessage}`);
    }
  }
